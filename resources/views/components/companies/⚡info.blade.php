...
new class extends Component {
    ...
    public int $simultaneous_services;
...
<div>
    <x-ts-card header="Informações do estabelecimento" class="detail g-2">
        ...
        <x-detail label="CNPJ" value="{{ $company->formated_cnpj }}" />
        <x-detail label="Telefone" value="{{ $company->formated_whatsapp }}" />
...
<script>
    this.$on('open-modal', () => {
        setTimeout(() => {
            $modalOpen('edit-info-modal');
        }, 100);
    });

    this.$on('close-modal', () => {
        $modalClose('edit-info-modal');
    });
</script>
